...
# Polarized Self-Attention 极化自注意力
...
# 3个new-FSEM对应不同个数的膨胀卷积，且膨胀率不同（new_FSEM）
class FSEM1(nn.Module):
    def __init__(self, nIn):
        super().__init__()
        ...
        self.conv4_1 = conv2d(nIn, nIn // 2, (3, 3), padding=2, dilation=(2, 2), act=False)
        self.conv4_2 = conv2d(nIn // 2, nIn // 4, (3, 3), padding=3, dilation=(3, 3), act=False)
        self.conv4_3 = conv2d(nIn // 4, nIn // 4, (3, 3), padding=4, dilation=(4, 4), act=False)

        self.conv5_1 = conv2d(nIn, nIn // 2, (3, 3), padding=3, dilation=(3, 3), act=False)
        self.conv5_2 = conv2d(nIn // 2, nIn // 4, (3, 3), padding=4, dilation=(4, 4), act=False)
        self.conv5_3 = conv2d(nIn // 4, nIn // 4, (3, 3), padding=5, dilation=(5, 5), act=False)

        self.conv6_1 = conv2d(nIn, nIn // 2, (3, 3), padding=4, dilation=(4, 4), act=False)
        self.conv6_2 = conv2d(nIn // 2, nIn // 4, (3, 3), padding=5, dilation=(5, 5), act=False)
        self.conv6_3 = conv2d(nIn // 4, nIn // 4, (3, 3), padding=6, dilation=(6, 6), act=False)
    ...
    def forward(self, x):
        ...
        o5_1 = self.conv5_1(x)
        o5_2 = self.conv5_2(o5_1)
        o5_3 = self.conv5_3(o5_2)

        o6_1 = self.conv6_1(x)
        o6_2 = self.conv6_2(o6_1)
        o6_3 = self.conv6_3(o6_2)

        o6 = torch.cat([o6_1, o6_2, o6_3], 1)
        o5 = torch.cat([o5_1, o5_2, o5_3], 1)

        o4 = torch.cat([o4_1, o4_2, o4_3], 1)
        o3_4 = torch.cat([o3_1, o3_2, o3_3], 1)
        o2_3 = torch.cat([o2_1, o2_2], 1)

        x_out = self.bn1(o6 + o5 + o4 + o3_4 + o2_3)
...
class FSEM2(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out = self.bn1(o5 + o4 + o3_4 + o2_3)
        x_out = self.selayer(x_out) + o1_2
        x_out = self.conv_out(x_out)
...
class out_conv_block(nn.Module):
    def __init__(self, ch_in, ch_out):
        super(out_conv_block, self).__init__()
        self.conv = nn.Sequential(
...
class CAFE(nn.Module):
    def __init__(self, channel=32):
        ...
        self.fsem1 = FSEM1(64)
        self.fsem2 = FSEM2(128)
        self.fsem3 = FSEM3(320)

        self.cbam1 = CBAM(in_planes=64)
        self.cbam2 = CBAM(in_planes=128)
        self.cbam3 = CBAM(in_planes=320)

        self.cbam1_out_conv_block = out_conv_block(64, 64)
        self.cbam2_out_conv_block = out_conv_block(128, 128)
        self.cbam3_out_conv_block = out_conv_block(320, 320)

        self.cbam_out1 = nn.Conv2d(64, 1, 1)
        self.cbam_out2 = nn.Conv2d(128, 1, 1)
        self.cbam_out3 = nn.Conv2d(320, 1, 1)
    ...
    def forward(self, x):
        ...
        x_1_cbam = self.cbam1(x_1)
        x_2_cbam = self.cbam2(x_2)
        x_3_cbam = self.cbam3(x_3)

        x_cbam1_out = self.cbam1_out_conv_block(x_1_cbam)
        x_cbam2_out = self.cbam2_out_conv_block(x_2_cbam)
        x_cbam3_out = self.cbam3_out_conv_block(x_3_cbam)

        cbam1_out = F.interpolate(self.cbam_out1(x_cbam1_out), scale_factor=4, mode='bilinear')
        cbam2_out = F.interpolate(self.cbam_out2(x_cbam2_out), scale_factor=8, mode='bilinear')
        cbam3_out = F.interpolate(self.cbam_out3(x_cbam3_out), scale_factor=16, mode='bilinear')
        ...
        out4 = F.interpolate(self.out4(x_4), scale_factor=32, mode='bilinear')

        return cbam1_out, cbam2_out, cbam3_out, out4, prediction1_4, prediction2_8, prediction3_16
...
if __name__ == '__main__':
    ...
    prediction1, prediction2, prediction3, prediction4, prediction5 = model(input_tensor)
    print(prediction1.size(), prediction2.size(), prediction3.size(), prediction4.size(), prediction5.size())
